...
async def invoke_our_graph(st_messages, st_placeholder):
    ...
    async for event in graph_runnable.astream_events({"messages": st_messages}, version="v2"):
        ...
        #        status_placeholder.status("Buscando Información...", expanded=False) 
                #with status_placeholder.status("Analizando problema...", expanded=False) as s:
                    #st.write("Leyendo ", event['name'])  # Show which tool is being called
        ...
        #            status_placeholder.status('hola') 
        ...
        #            status_placeholder.write('hola', expanded=False)
        #            output_placeholder.write('listo')  # Display the tool's output
